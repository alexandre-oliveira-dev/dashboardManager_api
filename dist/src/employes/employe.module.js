"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.EmployeModule = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const employes_1 = require("../schemas/employes");
const employe_service_1 = require("./employe.service");
const employe_controller_1 = require("./employe.controller");
let EmployeModule = class EmployeModule {
};
exports.EmployeModule = EmployeModule;
exports.EmployeModule = EmployeModule = __decorate([
    (0, common_1.Module)({
        imports: [
            mongoose_1.MongooseModule.forFeature([
                { name: employes_1.Employes.name, schema: employes_1.EmployesSchema },
            ]),
        ],
        providers: [employe_service_1.EmployesService],
        controllers: [employe_controller_1.EmployesController],
    })
], EmployeModule);
//# sourceMappingURL=employe.module.js.map